...
| **Co-change tables, `cochange_pairs_above_threshold`, `static_file_edges`** | **Keep.** Consumers: review `behavioral_coupling`, component clustering | The one git signal already shaped to the diff. 476 fixed the bulk-commit, `mod`-edge and single-commit noise, but it names 0 of 4 additive-PAR misses: 3 of them are in the diff's own file ([behavioral-coupling-backtest.md](behavioral-coupling-backtest.md)). |
