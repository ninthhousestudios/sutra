...
// ── Convention drift detection tests ────────────────────────────────

#[test]
fn fca_conformance_basic() {
    use sutra::conventions::{Convention, SymbolAttrs};
    use sutra::health::drift::compute_fca_conformance;
...
#[test]
fn fca_conformance_no_antecedent_matches() {
    use sutra::conventions::{Convention, SymbolAttrs};
    use sutra::health::drift::compute_fca_conformance;
...
#[test]
fn hrr_coherence_identical_vectors() {
    use sutra::health::drift::compute_hrr_coherence;
    ...
    let s3 = seed_fn(&db, fid, "a::f3", "f3", None);

    let vec_data: Vec<f64> = (0..1024).map(|i| (i as f64) * 0.001).collect();
    let bytes: Vec<u8> = vec_data.iter().flat_map(|f| f.to_le_bytes()).collect();
    let vectors: Vec<(i64, &str, &[u8])> = vec![
        (s1, "strip", bytes.as_slice()),
        (s2, "strip", bytes.as_slice()),
        (s3, "strip", bytes.as_slice()),
    ];
...
#[test]
fn hrr_coherence_too_few_symbols() {
    use sutra::health::drift::compute_hrr_coherence;
...
fn drift_detection_fca_drop_fires() {
    ...
    assert!(findings[0].metric_value > FCA_DRIFT_THRESHOLD);
    assert_eq!(findings[0].severity, HealthSeverity::Informational);
}

#[test]
fn drift_detection_below_threshold_no_finding() {
    use std::collections::HashMap;
    use sutra::conventions::SymbolAttrs;
    use sutra::health::drift::detect_convention_drift;
...
#[test]
fn drift_detection_no_history_no_finding() {
    use std::collections::HashMap;
    use sutra::conventions::SymbolAttrs;
    use sutra::health::drift::detect_convention_drift;
...
#[test]
fn drift_detection_both_paths_independent() {
    use std::collections::HashMap;
    use sutra::conventions::SymbolAttrs;
    use sutra::health::drift::detect_convention_drift;
...
#[test]
fn drift_detection_hrr_drop_fires() {
    use std::collections::HashMap;
    use sutra::conventions::SymbolAttrs;
    use sutra::health::drift::detect_convention_drift;
...
#[test]
fn convention_snapshot_stores_drift_metrics() {
    let (_dir, db) = setup_db();
    db.insert_component("comp1", "TestComp").unwrap();
    ...
    assert_eq!(snaps.len(), 1);
    assert!((snaps[0].fca_conformance.unwrap() - 0.92).abs() < 1e-9);
    assert!((snaps[0].hrr_coherence.unwrap() - 0.78).abs() < 1e-9);
}

#[test]
fn convention_snapshot_null_drift_metrics() {
    let (_dir, db) = setup_db();
    db.insert_component("comp1", "TestComp").unwrap();
