...
fn setup_linear_chain() -> (tempfile::TempDir, Db) {
    ...
    db.upsert_file("src/main.rs", "rust", "h1", 100, true)
        .unwrap();
    db.upsert_file("src/process.rs", "rust", "h2", 50, true)
        .unwrap();
    db.upsert_file("src/helper.rs", "rust", "h3", 30, true)
        .unwrap();
    ...
    db.insert_symbol(&sym(f1.id, "main", "main", "function", 1, 20))
        .unwrap();
    db.insert_symbol(&sym(f2.id, "mod::process", "process", "function", 1, 15))
        .unwrap();
    db.insert_symbol(&sym(f3.id, "mod::helper", "helper", "function", 1, 10))
        .unwrap();

    let process_sym = db
        .symbol_by_qualified_name("mod::process")
        .unwrap()
        .unwrap();
    let helper_sym = db.symbol_by_qualified_name("mod::helper").unwrap().unwrap();

    // main calls process at line 10
    db.insert_ref(f1.id, Some(process_sym.id), None, 10, 0, "call")
        .unwrap();
    // process calls helper at line 5
    db.insert_ref(f2.id, Some(helper_sym.id), None, 5, 0, "call")
        .unwrap();
...
fn cycle_detected_and_marked() {
    ...
    db.insert_symbol(&sym(fa.id, "mod::alpha", "alpha", "function", 1, 20))
        .unwrap();
    db.insert_symbol(&sym(fb.id, "mod::beta", "beta", "function", 1, 20))
        .unwrap();
    ...
    db.insert_ref(fa.id, Some(beta.id), None, 5, 0, "call")
        .unwrap();
    ...
    assert!(
        has_any_cycle,
        "cycle between alpha and beta should be detected"
    );

    let cycle_path = paths
        .iter()
        .find(|p| p["has_cycle"].as_bool() == Some(true))
        .unwrap();
    assert!(
        cycle_path["cycle_at"].as_str().is_some(),
        "cycle_at should name the cycle target"
    );
}
...
fn path_limit_respected() {
    ...
    db.insert_symbol(&sym(ft.id, "mod::target", "target", "function", 1, 10))
        .unwrap();
    let target = db.symbol_by_qualified_name("mod::target").unwrap().unwrap();

    for i in 0..20 {
        let path = format!("src/caller_{i}.rs");
        db.upsert_file(&path, "rust", &format!("hc{i}"), 20, true)
            .unwrap();
        let f = db.file_by_path(&path).unwrap().unwrap();
        let qn = format!("mod::caller_{i}");
        let sn = format!("caller_{i}");
        db.insert_symbol(&sym(f.id, &qn, &sn, "function", 1, 10))
            .unwrap();
        db.insert_ref(f.id, Some(target.id), None, 5, 0, "call")
            .unwrap();
    }
