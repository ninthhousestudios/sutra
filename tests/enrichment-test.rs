...
fn seed_file_with_imports(db: &Db) -> i64 {
    let file_id = db
        ...
        .unwrap();
    db.insert_import(file_id, "rusqlite::params", None, 1, "use", None)
        .unwrap();
    db.insert_import(file_id, "rusqlite::Connection", None, 2, "use", None)
        .unwrap();
    db.insert_import(file_id, "serde_json::json", None, 3, "use", None)
        .unwrap();
    file_id
}
...
fn rust_internal_imports_skipped() {
    ...
    let file_id = db
        ...
        .unwrap();
    db.insert_import(file_id, "crate::error::Result", None, 1, "use", None)
        .unwrap();
    db.insert_import(file_id, "self::helpers::parse", None, 2, "use", None)
        .unwrap();
    db.insert_import(file_id, "super::config::Config", None, 3, "use", None)
        .unwrap();
    db.insert_import(file_id, "std::collections::HashMap", None, 4, "use", None)
        .unwrap();
    db.insert_import(file_id, "rusqlite::Connection", None, 5, "use", None)
        .unwrap();
