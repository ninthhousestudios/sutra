...
pub fn resolve_python_imports(db: &Db, workspace_root: &Path) -> Result<usize> {
    ...
    for (import_id, file_id, raw_path) in &unresolved {
        let resolved = if raw_path.starts_with('.') {
            ...
            resolve_absolute(raw_path, &package_roots, &path_to_id)
        };
...
fn discover_package_roots(files: &[crate::db::FileRow], workspace_root: &Path) -> Vec<String> {
    ...
    roots.into_iter().map(|s| s.to_string()).collect()
}
...
fn resolve_absolute(
    ...
    for root in package_roots {
        if let Some(id) = try_resolve_segments(root, &segments, path_to_id) {
            return Some(id);
        }
    }
    None
...
mod tests {
    ...
    fn absolute_foo_bar_resolves_to_py_file() {
        ...
        assert_eq!(resolve_absolute("foo.bar", &roots, &path_to_id), Some(2));
    }
    ...
    fn package_resolves_to_init() {
        ...
        let roots = vec![String::new()];
        assert_eq!(resolve_absolute("foo", &roots, &path_to_id), Some(1));
    }
    ...
    fn unresolved_external_returns_none() {
        let (path_to_id, _) = make_path_map(&["foo/__init__.py", "foo/bar.py"]);
        let roots = vec![String::new()];
        assert_eq!(resolve_absolute("pathlib", &roots, &path_to_id), None);
        assert_eq!(resolve_absolute("os.path", &roots, &path_to_id), None);
    }
    ...
    fn src_layout_with_configured_roots() {
        ...
        assert_eq!(
            resolve_absolute("mypackage.module", &roots, &path_to_id),
            Some(2)
        );
    }
